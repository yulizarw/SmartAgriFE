...
export const Irrigation = ({
  ...
  const listCrop = useSelector((state) => {
    if (Array.isArray(state?.crop?.listCrop)) {
      return state.crop.listCrop;
    }

    if (Array.isArray(state?.crops?.listCrop)) {
      return state.crops.listCrop;
    }
  ...
  useEffect(() => {
    ...
    setError("");
  }, [farmId, cropId, selectedCrop, selectedFarm, historyLimit]);
  ...
  const sensorStatus = useMemo(() => {
    ...
    const iotScore = cropHealth.iotScore;

    if (iotScore !== null && iotScore !== undefined && iotScore !== "") {
      return "ONLINE";
    }
  ...
  const decisionDataMode = useMemo(() => {
  ...
  }, [decisionConfidence, sensorStatus]);

  // ============================================================
  // SAFETY APPLIED
  // ============================================================

  const safetyApplied = useMemo(() => {
    if (!recommendation || !decision) {
      return false;
    }

    const wantsWater = toBoolean(recommendation.watering);

    const finalDecision = String(decision?.decision || "")
      .trim()
      .toUpperCase();
    ...
    return (
      wantsWater &&
      finalDecision === "WATERING_OFF" &&
      (source === "AUTO" || source === "AUTOMATIC")
    );
  ...
  const handleHistoryPrevious = () => {
    if (historyLoading || !historyPagination.hasPrevPage) {
      return;
    }
  ...
  const handleHistoryNext = () => {
    if (historyLoading || !historyPagination.hasNextPage) {
      return;
    }
    ...
    <>
      ...
      <main className="irrigation-page">
        ...
        <section className="irrigation-control-bar">
          ...
          <div className="irrigation-control-info">
            <span className="irrigation-label">SENSOR STATUS</span>

            <strong
              className={
                sensorStatus === "ONLINE"
                  ? "irrigation-sensor-online"
                  : "irrigation-sensor-offline"
              }
            >
              {sensorStatus === "ONLINE"
                ? "🟢 ONLINE"
          ...
          <div className="irrigation-control-info">
            <span className="irrigation-label">CONFIDENCE</span>

            <strong
              className={`irrigation-confidence ${
                decisionConfidence !== null && decisionConfidence >= 80
                  ? "confidence-high"
                  : decisionConfidence !== null && decisionConfidence >= 70
                    ? "confidence-medium"
                    : "confidence-low"
              }`}
            >
              {decisionConfidence !== null ? `${decisionConfidence}%` : "-"}
            </strong>
          </div>
          ...
          <div className="irrigation-control-info">
            <span className="irrigation-label">DATA MODE</span>

            <strong>
              {decisionDataMode === "IOT PRIMARY"
                ? "📡 IOT PRIMARY"
                : decisionDataMode === "FALLBACK STRONG"
                  ? "🛰 FALLBACK STRONG"
                  : decisionDataMode === "FALLBACK MEDIUM"
                    ? "🛰 FALLBACK MEDIUM"
                    : decisionDataMode === "FALLBACK WEAK"
                      ? "⚠ FALLBACK WEAK"
                      : "-"}
            </strong>
          </div>
          ...
          <div className="irrigation-control-info">
            <span className="irrigation-label">SAFETY</span>

            <strong
              className={
                safetyApplied
                  ? "irrigation-safety-active"
                  : "irrigation-safety-normal"
              }
            >
              {safetyApplied ? "🛡 SAFETY APPLIED" : "✓ NORMAL"}
            </strong>
          </div>
          ...
          <div className="irrigation-control-info">
            <span className="irrigation-label">AUTOMATION</span>

            <strong>
              {automationStatus === "BLOCKED"
                ? "🛑 BLOCKED"
                : automationStatus === "SAFETY"
                  ? "🛡 SAFETY MODE"
                  : "🤖 ACTIVE"}
            </strong>
          </div>
        ...
        {decision && (
          <section className="irrigation-decision-reason-card">
            <div className="irrigation-decision-reason-header">
              <div>
                <span className="irrigation-card-label">DECISION ENGINE</span>

                <h3>Decision Explanation</h3>
              </div>

              <span
                className={`decision-engine-status ${
                  safetyApplied ? "safety" : "normal"
                }`}
              >
                {safetyApplied ? "🛡 SAFETY" : "✓ NORMAL"}
              </span>
            ...
            <p className="irrigation-decision-reason-text">
              {decision?.reason || "Belum tersedia penjelasan decision."}
            </p>

            <div className="decision-engine-meta">
              <div>
                <span>Source</span>
              ...
              <div>
                ...
                <strong>{decisionDataMode}</strong>
              </div>
          ...
          <div className="irrigation-flow">
            ...
            <div className="flow-step">
              ...
              <small>
                {sensorStatus === "ONLINE"
                  ? `ONLINE • Score ${
                      decision?.sensorValue ?? cropHealth?.iotScore ?? "-"
                    }`
                  : "OFFLINE"}
              </small>
            </div>
            ...
            <div className="flow-step">
              ...
              <small>
                {decisionConfidence !== null
                  ? `${decisionConfidence}% • ${decisionDataMode}`
                  : "-"}
              </small>
            </div>
...
function detectControlMode(relay, decision) {
  ...
  if (
    ...
    normalized.includes("RESUME_AUTO")
  ) {
    return "AUTO";
  }
